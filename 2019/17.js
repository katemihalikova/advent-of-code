...
function part1(program) {
  ...
  let grid = output.map(code => String.fromCharCode(code)).join('').split('\n').map(line => line.split(''));

  let getThingsAround = (x, y) => [grid[y][x], grid[y][x - 1], grid[y][x + 1], grid[y - 1] && grid[y - 1][x], grid[y + 1] && grid[y + 1][x]];

  return grid
    .flatMap((row, y) => row.map((_, x) => getThingsAround(x, y).every(thing => thing === '#') ? x * y : 0))
    .reduce((sum, alignmentParameter) => sum + alignmentParameter);
}

// == PART 2 ==

// pattern finding done manually
